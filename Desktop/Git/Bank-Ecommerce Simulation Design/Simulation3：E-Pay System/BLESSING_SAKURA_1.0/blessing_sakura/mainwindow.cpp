...
void MainWindow::on_pushButton_20_clicked()
{
    ...
    connect(socket,SIGNAL(connected()),this,SLOT(send2()));
    connect(socket,SIGNAL(readyRead()),this,SLOT(readMessage()));
    connect(socket,SIGNAL(error(QAbstractSocket::SocketError)),this,SLOT(ee(QAbstractSocket::SocketError)));
}

void MainWindow::send2()
{
    QString cid,code,pay;
    cid=ui->comboBox->currentText();
    code=ui->lineEdit_7->text();
    pay=ui->label_sum->text();
    QString s="*"+cid+code+pay;
...
void MainWindow::on_pushButton_29_clicked()
{
    socket= new QTcpSocket;
    socket->connectToHost(QHostAddress("127.0.0.1") ,6666);
    connect(socket,SIGNAL(connected()),this,SLOT(send1()));
    connect(socket,SIGNAL(readyRead()),this,SLOT(readMessage()));
    connect(socket,SIGNAL(error(QAbstractSocket::SocketError)),this,SLOT(ee(QAbstractSocket::SocketError)));

}

void MainWindow::ee(QAbstractSocket::SocketError)
{
    QMessageBox::warning(NULL,"warning",socket->errorString(),QMessageBox::Cancel);
}

void MainWindow::send1()
{
    QString cid,code;
    cid=ui->lineEdit_5->text();
...
void MainWindow::readMessage()
{
   ...
   fp='p';
   ue='u';
   if(!QString::compare(s.mid(0,1),sw))
   {
       if(!QString::compare(s.mid(1),ss))
        {
           ui->stackedWidget->setCurrentIndex(11);
           QFile f("client/"+ui->lineEdit->text()+"/bank.txt");
           ...
           {
               QTextStream inp(&f);
               inp<<ui->lineEdit_5->text()<<endl;
               inp<<"**********"<<endl;
               ui->stackedWidget->setCurrentIndex(11);
           }
         }
           else if(!QString::compare(s.mid(1),fc))
       {
           QMessageBox::warning(NULL,"warning","NO CLIENT INFO",QMessageBox::Cancel);
       }
           else if(!QString::compare(s.mid(1),fp))
       {
           QMessageBox::warning(NULL,"warning","WRONG CODE ",QMessageBox::Cancel);
           ui->lineEdit_5->setText("");
           ui->lineEdit_6->setText("");
   ...
   else if(!QString::compare(s.mid(0,1),sc))
   {
       if(!QString::compare(s.mid(1),ss))
        {
           acgn *a;
       ...
       else if(!QString::compare(s.mid(1),ue))
       {
           QMessageBox::warning(NULL,"warning","小鬼你没那么多钱",QMessageBox::Cancel);
       }
   }
